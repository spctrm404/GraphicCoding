...
function resizeHandler(entries) {
  for (let entry of entries) {
    ...
    setWalls(Math.floor(newWidth), Math.floor(newHeight));
    console.log(walls);
  }
}

function setWalls(newWidth, newHeight) {
  Body.setPosition(walls[0], { x: newWidth * 0.5, y: 0 });
  Body.setVertices(walls[0], [
    { x: 0, y: -wallThickness / 2 },
    { x: newWidth, y: -wallThickness / 2 },
    { x: newWidth, y: wallThickness / 2 },
    { x: 0, y: wallThickness / 2 },
  ]);

  Body.setPosition(walls[1], { x: newWidth * 0.5, y: newHeight });
  Body.setVertices(walls[1], [
    { x: 0, y: newHeight - wallThickness / 2 },
    { x: newWidth, y: newHeight - wallThickness / 2 },
    { x: newWidth, y: newHeight + wallThickness / 2 },
    { x: 0, y: newHeight + wallThickness / 2 },
  ]);

  Body.setPosition(walls[2], { x: 0, y: newHeight * 0.5 });
  Body.setVertices(walls[2], [
    { x: -wallThickness / 2, y: 0 },
    { x: wallThickness / 2, y: 0 },
    { x: wallThickness / 2, y: newHeight },
    { x: -wallThickness / 2, y: newHeight },
  ]);

  Body.setPosition(walls[3], { x: newWidth, y: newHeight * 0.5 });
  Body.setVertices(walls[3], [
    { x: newWidth - wallThickness / 2, y: 0 },
    { x: newWidth + wallThickness / 2, y: 0 },
    { x: newWidth + wallThickness / 2, y: newHeight },
    { x: newWidth - wallThickness / 2, y: newHeight },
  ]);
}
